...
const createFolder = (path: string, window: VSCodeWindow): void => {
  ...
  try {
    existsSync(path)
      ? window.showWarningMessage(`Folder ${folderName} already exists`)
      : mkdirSync(path);
  } catch (err) {
    window.showErrorMessage(
      `Something went wrong creating ${folderName} folder`
    );
  }
};
...
const createFile = (path: string, contents: string, window: VSCodeWindow) => {
  ...
  try {
    existsSync(path)
      ? window.showWarningMessage(`File ${fileName} already exists`)
